...
function Education() {
  ...
  const fetchProjects = async () => {
    try {
      ...
      const educationProjects = response.data.filter(
        (project) => project.category === "Education"
      );
      setProjects(educationProjects);
      setLoading(false);
    } catch (error) {
  ...
  const renderProjectSection = (status, title) => {
    ...
    return (
      <div className="mb-5">
        <h3 className="mb-4" style={{ color: "#198754", borderBottom: "2px solid #198754", paddingBottom: "10px" }}>
          {title}
        </h3>
        <Row className="g-4">
          ...
          <div className="mb-4">
            ...
            {loading ? (
              ...
              <div className="text-center py-5 text-muted">
                <p>No education projects found. Check back soon!</p>
              </div>
            ) : (
              <>
